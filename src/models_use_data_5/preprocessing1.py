...
import process_var1256 as proc
import pandas as pd
...
for curr_var in columns_hasnan:
    ...
    all_features_df[curr_var] = tmp[:]
    columns_hasnan = columns_hasnan.drop([curr_var])
assert columns_hasnan.shape[0]==0
...
print('处理var124-126')
var124 = proc.process(all_features_df['var124'].str.replace(' ', '')) \
    .apply(lambda str: '吉林' if str == '' else str)
var125 = proc.process(all_features_df['var125'])
var126 = proc.process(all_features_df['var126'])

var12456 = var124.str.strip() + ' ' \
           + var125.str.strip() + ' ' \
           + var126.str.strip()

# tfidf
count_vect = CountVectorizer()
count_var12456 = count_vect.fit_transform(var12456.values)
tfidf_transformer = TfidfTransformer()
tfidf_var12456 = tfidf_transformer.fit_transform(count_var12456)
tfidf_var12456_pd = pd.DataFrame(tfidf_var12456.toarray())

print('PCA降维...')
pca = PCA(n_components=200, svd_solver='full', random_state=36)
pca.fit(tfidf_var12456_pd)
var12456_col = ['var12456_' + str(i) for i in range(0, 200, 1)]
var12456_features = pd.DataFrame(pca.transform(tfidf_var12456_pd), columns=var12456_col)
all_features_df = all_features_df.join(var12456_features)
...
print('数值型数据标准化...')
numerical_columns = (all_features_df.columns).drop(categorical_index).drop(var12456_col).drop('id')
...
col_var19 = ['var19_' + str(i) for i in range(0, tfidf_var19.shape[1], 1)]
# 存入特征dataframe中
...
pca = PCA(n_components=100,
          svd_solver='full',
          random_state=36)
pca_features = pca.fit_transform(tfidf_var_shi)

shi_features = pd.DataFrame(pca_features,columns=['var_shi_' + str(i) for i in range(0, pca_features.shape[1], 1)])
...
# ---------保存预处理后的数据----------------
print("存储数据...")
print('位置：../../data/data_5/train_preprocessed1.csv & test_preprocessed1.csv')
...
train_df.to_csv('../../data/data_5/train_preprocessed1.csv', encoding='gbk', index=False)
test_df.to_csv('../../data/data_5/test_preprocessed1.csv', encoding='gbk', index=False)
